<script lang="ts">
	...
	import { getRoleDisplayName, getRoleColor } from '$lib/constants/roles';
...
<svelte:head>
	<title>Team-Verwaltung - Admin | TitanNode</title>
</svelte:head>
...
<div class="space-y-8">
	<div>
		...
		<p class="mt-2 text-sm sm:text-base text-muted-foreground">Verwalte Team-Mitglieder und Support-Pins</p>
	</div>
						...
						<div class="flex items-start gap-4 flex-1">
							...
							<div class="flex-1 space-y-3">
								...
								<div class="flex flex-wrap items-center gap-2">
									<Badge variant="outline" class={getRoleColor((member as { position?: string | null }).position ?? member.role)}>
										{getRoleDisplayName((member as { position?: string | null }).position ?? member.role)}
									</Badge>
						...
						<div class="flex flex-col gap-2">
							<form method="POST" action="?/regeneratePin" use:enhance>
								...
								<Button type="submit" variant="outline" size="sm" class="w-full">
									<Icon icon="tabler:refresh" class="mr-2 h-4 w-4" />
									PIN neu generieren
								</Button>
							</form>
