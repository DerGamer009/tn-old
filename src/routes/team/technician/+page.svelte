...
<div class="space-y-8">
		...
		<div class="flex flex-wrap items-center justify-between gap-4">
			<div>
				...
				<p class="mt-1 text-sm sm:text-base text-muted-foreground">
					Übersicht des technischen Teams: Rollen, Zuständigkeiten und Team-Mitglieder.
				</p>
			</div>
		</div>
		{#if data.stats}
	...
	<!-- Technische Rollen (aus team-structure) -->
	<Card class="border-border/60 bg-card/90 backdrop-blur">
		<CardHeader>
			<CardTitle class="text-lg flex items-center gap-2">
				<Icon icon="tabler:list-details" class="h-5 w-5 text-primary" />
				Technisches Team – Rollen & Zuständigkeiten
			</CardTitle>
			<p class="text-sm text-muted-foreground">
				Alle Rollen des technischen Bereichs (Head of Infrastructure, Senior/Junior Technician, System/Network Admin, Security, DevOps, Monitoring, Backup & Recovery).
			</p>
		</CardHeader>
		<CardContent>
			<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
				{#each data.technicalRoles as role}
					<div
						class="rounded-xl border border-border/50 bg-muted/30 p-4 transition-colors hover:bg-muted/50"
					>
		...
		<CardContent>
			{#if data.technicians.length === 0}
				<p class="py-8 text-center text-muted-foreground">
					Noch keine Techniker im System zugeordnet.
				</p>
			{:else}
				<div class="space-y-2">
					{#each data.technicians as member}
						<div
							class="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-border/50 p-4"
						>
							<div class="flex items-center gap-3 min-w-0">
								...
								<div class="min-w-0">
									<div class="flex flex-wrap items-center gap-2">
										...
										<Badge
											variant="outline"
											class="{ROLE_COLORS[member.role]} text-xs"
										>
