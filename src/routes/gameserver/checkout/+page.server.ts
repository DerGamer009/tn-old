...
export const load: PageServerLoad = async ({ parent, url }) => {
	...
	const nestId = url.searchParams.get('nestId'); // Optional: 6 = Minecraft Java, 7 = Minecraft Bedrock (Default 6)
	const locationId = url.searchParams.get('locationId');
	const price = url.searchParams.get('price');
	const javaVersion = url.searchParams.get('javaVersion'); // Optional: Docker-Image für Minecraft-Java

	if (!name || !cpu || !ram || !storage || !slots || !game || !eggId || !locationId || !price) {
		throw redirect(302, '/gameserver/konfigurator');
	}
	...
	return {
		...
		nestId: nestId ? parseInt(nestId, 10) : 6,
...
export const actions = {
	processPayment: async ({ request, locals, url }) => {
		...
		const slots = data.get('slots') as string;
		const game = data.get('game') as string;
		const eggId = data.get('eggId') as string;
		const locationId = data.get('locationId') as string;
		const nestIdStr = data.get('nestId') as string | null;
		const price = data.get('price') as string;
		const javaVersion = data.get('javaVersion') as string | null;

		if (!paymentMethod || !name || !cpu || !ram || !storage || !slots || !game || !eggId || !locationId || !price) {
		...
		try {
			if (paymentMethod === 'paypal') {
				...
				const orderId = `gameserver-${Date.now()}-${locals.user.id}`;
				
				const paypalUrl = await createPayPalOrder(priceNum, orderId, locals.user.id, 'Gameserver');
				...
				await prisma.order.create({
					...
					metadata: JSON.stringify({
						...
						game,
						eggId: parseInt(eggId, 10),
						nestId: nestIdStr ? parseInt(nestIdStr, 10) : 6,
						locationId: parseInt(locationId, 10),
						dockerImage: javaVersion || undefined
					})
				}
			});
				...
				const klarnaSession = await createKlarnaSession(priceNum, orderId, locals.user, 'Gameserver');
				...
				throw redirect(302, `/gameserver/checkout/success?orderId=${orderId}`);
			}
		...
		`https://api-m.${paypalMode === 'sandbox' ? 'sandbox.' : ''}paypal.com/v2/checkout/orders`,
		{
			...
			body: JSON.stringify({
				...
				application_context: {
					...
					user_action: 'PAY_NOW',
					return_url: `${process.env.PUBLIC_APP_URL || process.env.PUBLIC_BASE_URL || 'http://localhost:5173'}/gameserver/checkout/success?orderId=${orderId}`,
					cancel_url: `${process.env.PUBLIC_APP_URL || process.env.PUBLIC_BASE_URL || 'http://localhost:5173'}/gameserver/checkout/cancel`
				}
			})
		}
	);
