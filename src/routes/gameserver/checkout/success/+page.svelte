...
<svelte:head>
	<title>Bestellung erfolgreich - Gameserver | TitanNode</title>
</svelte:head>
...
<div class="max-w-2xl mx-auto space-y-6">
			...
			<div class="text-center space-y-4">
				...
				<div>
					...
					<p class="text-muted-foreground mt-2">
						Dein Gameserver wird jetzt erstellt und ist in wenigen Minuten verfügbar.
					</p>
				</div>
			</div>
		</CardContent>
	...
	<div class="flex flex-col sm:flex-row gap-3">
		{#if data.server}
			<Button class="flex-1" onclick={() => goto(`/dashboard/gameserver/${data.server.id}`)}>
				<Icon icon="tabler:server" class="mr-2 h-4 w-4" />
				Zum Gameserver
			</Button>
		{/if}
		<Button variant="outline" class="flex-1" onclick={() => goto('/dashboard/gameserver')}>
