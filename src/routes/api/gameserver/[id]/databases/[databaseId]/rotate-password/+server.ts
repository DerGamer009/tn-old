...
export const POST: RequestHandler = async ({ locals, params, request }) => {
	...
	try {
		...
		// Rotiere Database-Passwort über Pterodactyl User API
		const response = await fetch(
			`${PTERODACTYL_API_BASE}/api/client/servers/${serverIdentifier}/databases/${params.databaseId}/rotate-password`,
			{
				method: 'POST',
				headers: {
		...
		const database = dbData.attributes || dbData.data?.attributes || dbData;
		const password = database.relationships?.password?.attributes?.password || null;

		// Log Activity
		await logServerActivity(
			server.id,
			locals.user.id,
			'database_rotate_password',
			getClientIp(request),
			`Passwort für Database "${database.name || params.databaseId}" rotiert`
		);
		...
		console.error('Passwort rotieren Fehler:', err);
		// Wenn es bereits ein error() ist, werfe es weiter
		if (err && typeof err === 'object' && 'status' in err) {
			throw err;
		}
		throw error(500, err instanceof Error ? err.message : 'Konnte Passwort nicht rotieren');
	}
};
