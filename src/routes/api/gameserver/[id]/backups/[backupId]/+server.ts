...
export const DELETE: RequestHandler = async ({ locals, params, request }) => {
	...
	try {
		...
		// Lösche Backup über Pterodactyl User API
		const response = await fetch(
			`${PTERODACTYL_API_BASE}/api/client/servers/${serverIdentifier}/backups/${params.backupId}`,
			{
				method: 'DELETE',
				headers: {
		...
		await logServerActivity(
			...
			'backup_delete',
			getClientIp(request),
			`Backup "${params.backupId}" gelöscht`
		);

		return json({ success: true });
	} catch (err) {
		console.error('Backup löschen Fehler:', err);
		throw error(500, err instanceof Error ? err.message : 'Konnte Backup nicht löschen');
	}
};
