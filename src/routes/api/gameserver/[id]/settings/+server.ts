...
export const GET: RequestHandler = async ({ locals, params }) => {
	...
	try {
		...
		// Hole Server-Ressourcen
		let resources = null;
		try {
			const resourcesResponse = await fetch(
				`${PTERODACTYL_API_BASE}/api/client/servers/${serverIdentifier}/resources`,
				{
					headers: {
						'Authorization': `Bearer ${PTERODACTYL_USER_KEY}`,
						'Content-Type': 'application/json',
			...
			if (resourcesResponse.ok) {
				...
				resources = resourcesData.attributes || resourcesData;
			}
		} catch (e) {
			console.warn('Konnte Server-Ressourcen nicht abrufen:', e);
		}
		...
		console.error('Server-Settings laden Fehler:', err);
		throw error(500, err instanceof Error ? err.message : 'Konnte Server-Settings nicht laden');
	}
};
