...
import { stopGameserver } from '$lib/server/app-hosting';
...
export const POST: RequestHandler = async ({ params, locals, request }) => {
	...
	try {
		...
		console.error('Error stopping Gameserver:', error);
		return json(
			{ error: error instanceof Error ? error.message : 'Fehler beim Stoppen des Gameservers' },
			{ status: 500 }
		);
	}
};
