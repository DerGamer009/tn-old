...
export const POST: RequestHandler = async ({ locals, params, request }) => {
	...
	try {
		...
		// Führe Schedule aus über Pterodactyl User API
		const response = await fetch(
			`${PTERODACTYL_API_BASE}/api/client/servers/${serverIdentifier}/schedules/${params.scheduleId}/execute`,
			{
				method: 'POST',
				headers: {
		...
		await logServerActivity(
			...
			'schedule_execute',
			getClientIp(request),
			`Schedule ${params.scheduleId} manuell ausgeführt`
		);

		return new Response(null, { status: 204 });
	} catch (err) {
		console.error('Schedule ausführen Fehler:', err);
		// Wenn es bereits ein error() ist, werfe es weiter
		if (err && typeof err === 'object' && 'status' in err) {
			throw err;
		}
		throw error(500, err instanceof Error ? err.message : 'Konnte Schedule nicht ausführen');
	}
};
