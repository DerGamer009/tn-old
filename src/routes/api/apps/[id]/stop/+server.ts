...
import { stopAppHosting } from '$lib/server/app-hosting';
...
export const POST: RequestHandler = async ({ params, locals }) => {
	...
	try {
		...
		await stopAppHosting(id);
		return json({ success: true });
	} catch (error) {
		console.error('Error stopping App Hosting:', error);
		return json(
			{ error: error instanceof Error ? error.message : 'Fehler beim Stoppen der App' },
			{ status: 500 }
		);
	}
};
