<script lang="ts">
	...
	async function loadFile() {
		...
		isLoading = true;
		try {
			const url = `/api/apps/${data.server.id}/files/read?file=${encodeURIComponent(data.filename)}&path=${encodeURIComponent(data.filePath)}`;
			...
			if (!response.ok) {
				...
				throw new Error(`Konnte Datei nicht lesen: ${response.status}`);
			}
	...
	async function saveFile() {
		...
		isSaving = true;
		try {
			const response = await fetch(`/api/apps/${data.server.id}/files/write`, {
				method: 'POST',
	...
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-3">
			<Button
				variant="outline"
				size="sm"
				onclick={() => goto(`/dashboard/apps/${data.server.id}/files`)}
			>
					...
					<div class="flex justify-end gap-2">
						<Button
							...
							onclick={() => {
								...
								goto(`/dashboard/apps/${data.server.id}/files`);
							}}
						>
							Abbrechen
						</Button>
