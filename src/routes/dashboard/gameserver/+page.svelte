...
const translations = {
	de: {
		...
		pageSubtitle: 'Verwalte deine Gameserver',
		createButton: 'Neuen Gameserver erstellen',
		emptyText: 'Du hast noch keine Gameserver',
		emptyCta: 'Jetzt Gameserver erstellen',
		tableServer: 'Server',
	...
	en: {
		...
		pageSubtitle: 'Manage your game servers',
		createButton: 'Create new game server',
		emptyText: 'You do not have any game servers yet',
		emptyCta: 'Create game server now',
		tableServer: 'Server',
...
function t(key: TKey): string {
	...
	if (typeof window !== 'undefined') {
		$effect(() => {
			console.log('Gameserver:', data.servers);
	...
	<Card class="border-border/60 bg-gradient-to-r from-primary/10 via-background to-primary/5">
		<CardContent class="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
			<div>
				<div class="flex items-center gap-3">
					<div class="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/15">
						<Icon icon="tabler:device-gamepad" class="h-5 w-5 text-primary" />
					</div>
			...
			<div class="flex flex-col items-stretch gap-2 sm:items-end">
				<Button class="gap-2 w-full sm:w-auto" href="/gameserver">
					<Icon icon="tabler:plus" class="h-4 w-4" />
					{t('createButton')}
				</Button>
				{#if data.servers && data.servers.length > 0}
					<p class="text-xs text-muted-foreground">
						{data.servers.filter((s) => s.status === 'ACTIVE').length}/{data.servers.length}
						{' '}
						{t('statusOnline')}
					</p>
				{/if}
		...
		<Card class="border-border/50">
			<CardContent class="flex flex-col items-center justify-center py-16">
				<div class="mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-muted">
					<Icon icon="tabler:device-gamepad-off" class="h-14 w-14 text-muted-foreground" />
				</div>
				<p class="mb-6 text-lg text-muted-foreground">{t('emptyText')}</p>
				<Button href="/gameserver">{t('emptyCta')}</Button>
			</CardContent>
		</Card>
	{:else}
		<!-- Server Table -->
		<Card class="border-border/50">
			<CardContent class="p-0 overflow-x-auto">
				<Table class="min-w-[640px]">
					<TableHeader>
						<TableRow>
							<TableHead>{t('tableServer')}</TableHead>
							...
							<TableRow>
									...
									<div class="flex flex-col gap-0.5">
										<div class="flex items-center gap-1.5">
											{#if getDaysUntilExpiry(server) !== null && getDaysUntilExpiry(server)! < 0}
												<span title={t('expired')} class="shrink-0">
													<Icon icon="tabler:alert-circle-filled" class="h-4 w-4 text-red-500" />
												</span>
											{:else if getDaysUntilExpiry(server) !== null && getDaysUntilExpiry(server)! < 7}
												<span title="Laufzeit endet bald" class="shrink-0">
													<Icon icon="tabler:alert-triangle-filled" class="h-4 w-4 text-amber-500" />
												</span>
											{/if}
								...
								<TableCell>
									<Button
										...
										href="/dashboard/gameserver/{server.id}"
									>
										{t('manage')}
									</Button>
								</TableCell>
