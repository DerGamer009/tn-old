<script lang="ts">
	...
	async function loadFiles(path?: string) {
		...
		loadingFiles = true;
		try {
			const response = await fetch(`/api/gameserver/${data.server.id}/files?path=${encodeURIComponent(path || currentPath)}`);
			if (!response.ok) {
	...
	async function deleteFile(file: any) {
		if (!confirm(`Möchtest du "${file.name}" wirklich löschen?`)) return;
		if (!data || !data.server || !(data.server as any).pterodactylServerId) return;

		try {
			const response = await fetch(`/api/gameserver/${data.server.id}/files/delete`, {
				method: 'DELETE',
	...
	function editFile(file: any) {
		...
		goto(`/dashboard/gameserver/${data.server.id}/files/edit/${filename}?path=${encodeURIComponent(currentPath)}`);
	}
	...
	async function handleUpload(event: Event) {
		...
		try {
			...
			const uploadResponse = await fetch(
				`/api/gameserver/${data.server.id}/files/upload/proxy`,
				{
					method: 'POST',
					body: formData
				}
			...
			if (!uploadResponse.ok) {
				const err = await uploadResponse.json().catch(() => ({ error: 'Unbekannter Fehler' }));
				const msg = err.error || err.message || 'Unbekannter Fehler';
				console.error('Upload Fehler:', err);
				alert(`Fehler beim Hochladen der Datei: ${msg}`);
				return;
			}
			...
			uploading = false;
			// Reset Input, damit derselbe Dateiname erneut gewählt werden kann
	...
	<div class="flex items-center justify-between">
		...
		<div class="flex gap-2">
			...
			<Button
				variant="outline"
				size="sm"
				onclick={() => goto(`/dashboard/gameserver/${data.server.id}`)}
			>
