<script lang="ts">
	...
	let { user } = $props<{ user: TechnicianUser }>();
	let mobileMenuOpen = $state(false);

	const currentPath = $derived($page.url.pathname);

	const navItems = [
		{ href: '/technician', icon: 'tabler:dashboard', label: 'Dashboard' },
		{ href: '/technician/team', icon: 'tabler:users-group', label: 'Team' },
		{ href: '/technician/servers', icon: 'tabler:server', label: 'Server' },
		{ href: '/technician/nodes', icon: 'tabler:server-2', label: 'Nodes' },
		{ href: '/technician/tickets', icon: 'tabler:ticket', label: 'Tickets' }
	];
			...
			<div class="flex h-full flex-col">
				<div class="flex h-16 items-center gap-2 border-b px-6">
					...
					<div class="flex flex-col">
						...
						<span class="text-[11px] text-muted-foreground">Techniker Panel</span>
					</div>
				</div>
	...
	<div class="flex h-full flex-col">
		<div class="flex h-16 items-center gap-3 border-b border-border/60 px-6">
			<div class="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10">
				<Icon icon="tabler:tool" class="h-5 w-5 text-primary" />
			</div>
			<div class="flex flex-col">
				<span
